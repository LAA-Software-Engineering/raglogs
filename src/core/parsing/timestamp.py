...
def parse_timestamp_field(value: str | int | float) -> Optional[datetime]:
    ...
    if isinstance(value, str):
        ...
        # it dominated the ingest hot path (#85 profile). fromisoformat and
        # dateutil agree on any valid ISO string, so this only reorders which
        # parser wins; anything fromisoformat rejects falls through unchanged.
        try:
