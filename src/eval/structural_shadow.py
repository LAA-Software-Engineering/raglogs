...
def shadow_result(case: EvalCase) -> ShadowResult:
    ...
    inp = structural_signals(spans, load_metrics_jsonl(case.metrics_path), case.window_start)
    signals = inp.signals
    ...
    hypotheses = build_hypotheses(signals, inp.edges, inp.edge_signals, inp.util_signals)
    if not hypotheses:
    ...
    result = resolve(partition(hypotheses, build_observables(signals, inp.edge_signals, inp.util_signals)))
