...
class TraceSpan(Base):
    ...
    __table_args__ = (
        ...
        # Silent-service evidence (#184) aggregates `count() GROUP BY service` over a (scope,
        # time-range) with no service predicate; the service-first index can't serve that. Covering
        # `service` makes the two per-window aggregates index-only (no heap fetch).
        Index("ix_trace_spans_scope_start", "scope", "start_time", postgresql_include=["service"]),
    )
