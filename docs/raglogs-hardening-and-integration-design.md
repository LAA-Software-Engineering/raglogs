...
**Config.** Per-adapter blocks under `ADAPTER_CLOUDWATCH_*`, `DATADOG_*`, `LOKI_*`, etc. Adapters are optional; absent credentials disable the adapter and surface a clear `/health` sub-status.
