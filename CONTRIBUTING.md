...
New adapters go in `src/adapters/`. Each adapter yields `ParsedLogLine` objects. The rest of the pipeline is fully source-agnostic. CloudWatch, Datadog, Loki, and Kubernetes log exports (`k8s`) are already supported.
